...
import { termsFor, quoteFor } from '@/lib/rental-terms';
...
export default function CarDetail() {
  ...
  if (!selectedCar) {
    return (
      <div className="mx-auto max-w-3xl px-6 py-24 text-center">
        <p className="text-lg text-ink-2">{d.carDetail.noneSelected}</p>
  ...
  const handleBook = async (e: React.FormEvent) => {
    ...
    if (!duration.ok) {
      showToast(duration.error ? d.rules[duration.error] : d.rules.invalidDates);
      return;
    }
    ...
    try {
      ...
      if (json.success) {
        ...
        showToast(fmt(d.carDetail.confirmedToast, { reference: json.data.reference }));
      }
    } catch {
      showToast(d.carDetail.bookingError);
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="mx-auto max-w-[1200px] px-6 pt-8 md:px-10">
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-1 text-[15px] text-ink-2">{d.common.orSimilar} &middot; {car.supplierName}</p>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
              {[
                { icon: Settings2, label: d.carDetail.specs.transmission, value: d.transmissions[car.transmission as keyof typeof d.transmissions] ?? car.transmission },
                { icon: Fuel, label: d.carDetail.specs.fuel, value: d.fuels[car.fuel as keyof typeof d.fuels] ?? car.fuel },
                { icon: Users, label: d.carDetail.specs.seats, value: String(car.seats) },
                { icon: DoorOpen, label: d.carDetail.specs.doors, value: String(car.doors) },
                { icon: Briefcase, label: d.carDetail.specs.bags, value: String(car.bags) },
                { icon: Wind, label: d.carDetail.specs.ac, value: car.ac ? d.common.yes : d.common.no },
              ].map((spec) => (
            ...
            <div className="mt-6 rounded-[16px] bg-petrol-700 p-5 text-paper">
              <p className="label-tight text-[11px] text-saffron-300">{d.carDetail.priceTotal}</p>
              <p className="nums mt-1 font-poster text-4xl">
                {duration.ok ? f.euro(quote.total) : `${f.euro(car.pricePerDay)}${d.common.perDay}`}
              </p>
              <p className="nums mt-1 text-[14px] text-petrol-100">
                {duration.ok
                  ? `${f.duration(duration.days)} · ${f.euro(car.pricePerDay)}${d.common.perDay}${
                      selectedPlanData
                        ? ` · ${fmt(d.carDetail.priceWithInsurance, { plan: (locale === 'en' && selectedPlanData.nameEn) || selectedPlanData.name })}`
                        : ''
                    }`
        ...
        <section className="mt-14">
          <h2 className="font-poster text-[clamp(1.6rem,3vw,2.3rem)] text-ink">{d.carDetail.conditionsTitle}</h2>
          <p className="mt-2 max-w-[62ch] text-[15px] leading-relaxed text-ink-2">{d.carDetail.conditionsSub}</p>
          <div className="mt-6">
        ...
        {confirmedBooking && (
          <div className="mt-10 rounded-[20px] border border-petrol-500 bg-petrol-50 p-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-poster-md text-xl text-petrol-700">{d.carDetail.confirmedTitle}</h2>
            </div>
            <dl className="mt-4 grid gap-3 text-[15px] sm:grid-cols-2">
              <div><dt className="inline text-ink-2">{d.carDetail.reference} : </dt><dd className="nums inline font-bold text-ink">{confirmedBooking.reference}</dd></div>
              <div><dt className="inline text-ink-2">{d.carDetail.car} : </dt><dd className="inline font-bold text-ink">{car.name}</dd></div>
              <div><dt className="inline text-ink-2">{d.carDetail.duration} : </dt><dd className="nums inline font-bold text-ink">{f.duration(duration.days)}</dd></div>
              <div><dt className="inline text-ink-2">{d.carDetail.total} : </dt><dd className="nums inline font-bold text-ink">{f.euro(confirmedBooking.totalPrice)}</dd></div>
            </dl>
          </div>
        )}
        ...
        <section className="mt-14">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <h2 className="font-poster text-[clamp(1.6rem,3vw,2.3rem)] text-ink">{d.carDetail.reduceTitle}</h2>
            <button onClick={() => setPage('insurance')} className="text-[14px] font-bold text-petrol-600 underline underline-offset-4 hover:text-petrol-700">
              {d.carDetail.understandInsurance}
            </button>
          </div>
          <p className="mt-2 max-w-[62ch] text-[15px] leading-relaxed text-ink-2">
            {fmt(d.carDetail.reduceSub, { amount: f.euro(terms.excess) })}
          </p>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-4">
              <label className={`cursor-pointer rounded-[16px] border-2 p-4 transition-colors duration-200 ${selectedPlan === '' ? 'border-petrol-600 bg-petrol-50' : 'border-ink/12 bg-paper hover:border-ink/25'}`}>
                <span className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-ink">{d.carDetail.noInsurance}</span>
                </span>
                ...
                <span className="mt-2 block text-[13px] leading-relaxed text-ink-2">
                  {fmt(d.carDetail.noInsuranceHelp, { amount: f.euro(terms.excess) })}
                </span>
              </label>
              ...
              {insurancePlans.map((plan) => (
                ...
                >
                  <span className="flex items-center gap-2">
                    ...
                    <span>
                      <span className="block font-bold text-ink">{(locale === 'en' && plan.nameEn) || plan.name}</span>
                      <span className="block text-[12px] text-ink-2">{(locale === 'en' && plan.descriptionEn) || plan.description}</span>
                    </span>
                  </span>
                  <span className="nums mt-2 block font-poster-md text-lg text-petrol-600">{f.euro(plan.dailyPrice)}{d.common.perDay}</span>
        ...
        <section className="mt-14 rounded-[20px] bg-paper p-6 md:p-8">
          <h2 className="font-poster text-[clamp(1.6rem,3vw,2.3rem)] text-ink">{d.carDetail.bookTitle}</h2>
          ...
          <form onSubmit={handleBook} className="mt-6 space-y-5">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label htmlFor="nom" className="label-tight mb-2 block text-[11px] text-ink-2">{d.carDetail.fullName}</label>
                <input id="nom" type="text" value={formName} onChange={(e) => setFormName(e.target.value)} placeholder={user?.name || d.carDetail.yourName} className={field} />
              </div>
              <div>
                <label htmlFor="mail" className="label-tight mb-2 block text-[11px] text-ink-2">{d.carDetail.email}</label>
                <input id="mail" type="email" value={formEmail} onChange={(e) => setFormEmail(e.target.value)} placeholder={user?.email || d.carDetail.emailPlaceholder} className={field} />
              </div>
              <div>
                <label htmlFor="tel" className="label-tight mb-2 block text-[11px] text-ink-2">{d.carDetail.phone}</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="d1" className="label-tight mb-2 block text-[11px] text-ink-2">{d.carDetail.start}</label>
                  <input id="d1" type="date" value={startDate} min={isoDatePlus(0)} onChange={(e) => setStartDate(e.target.value)} className={field} />
                </div>
                <div>
                  <label htmlFor="d2" className="label-tight mb-2 block text-[11px] text-ink-2">{d.carDetail.end}</label>
              ...
              <div className="rounded-[16px] bg-paper-2 p-5">
                <dl className="space-y-2 text-[15px]">
                  <div className="flex justify-between text-ink-2">
                    <dt>{fmt(d.carDetail.lineItem, { car: car.name, duration: f.duration(duration.days) })}</dt>
                    <dd className="nums">{f.euro(quote.carTotal)}</dd>
                  </div>
                  {selectedPlanData && (
                    <div className="flex justify-between text-ink-2">
                      <dt>{fmt(d.carDetail.insuranceLine, { plan: (locale === 'en' && selectedPlanData.nameEn) || selectedPlanData.name, duration: f.duration(duration.days) })}</dt>
                      <dd className="nums">{f.euro(quote.insuranceTotal)}</dd>
                    </div>
                  )}
                  <div className="flex justify-between border-t border-ink/12 pt-2.5 text-lg font-bold text-ink">
                    <dt>{d.carDetail.totalToPay}</dt>
                    <dd className="nums text-petrol-600">{f.euro(quote.total)}</dd>
                  </div>
                </dl>
                <p className="mt-3 border-t border-ink/12 pt-3 text-[13px] leading-relaxed text-ink-2">
                  {fmt(d.carDetail.depositNote, { amount: f.euro(terms.deposit) })}
                </p>
              </div>
            )}
